...
public class ManageApp {
    ...
    @VariableReplay var foldersRoot: [FolderModel] = []
    @VariableReplay var files: [FolderModel] = []
    @VariableReplay var filesHome: [FolderModel] = []
    @VariableReplay var pinHomes: [FolderModel] = []
    public var refreshValue: PublishSubject<Void> = PublishSubject.init()
    ...
    private func defaultValueFolders(isFirstApp: Bool, icons: [String], folders: [String]) {
        if isFirstApp {
            Observable.zip(Observable.just(icons), Observable.just(folders)).bind { [weak self] item in
                guard let self = self else { return }
                let folder = item.1
                var l: [FolderModel] = []
    ...
    public func changefile(old: String, new: String, failure: ((String) -> Void)?) {
        ...
        self.createFolder(path: new) { outputURL in
            do {
                ...
                if let index = ManageApp.shared.folders.firstIndex(where: { $0.url.getNamePath().uppercased().contains(oldPath.getNamePath().uppercased()) }) {
